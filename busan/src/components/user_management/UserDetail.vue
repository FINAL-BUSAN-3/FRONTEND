...
export default {
  ...
  methods: {
    async fetchUser(userId) {
      try {
        const response = await axios.get(`http://localhost:8000/user-management/user-detail/${userId}`);
    ...
    async saveUser() {
      try {
        ...
        await axios.put(`http://localhost:8000/user-management/user-detail/${this.user.employeeNo}`, updatedUser);
