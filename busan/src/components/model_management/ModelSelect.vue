...
export default {
  ...
  methods: {
    async fetchModelSelectForManagement() {
      try {
        const response = await axios.get('http://127.0.0.1:8000/model-management/model-select');
    ...
    async selectModelForManagement(model_info_id) {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/model-management/model-info/${model_info_id}`);
