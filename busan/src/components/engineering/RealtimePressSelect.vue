...
export default {
  ...
  methods: {
    async fetchPrediction() {
      try {
        const response = await axios.get('http://localhost:8000/engineering/realtime-press/select');
        this.predictionData = response.data.press_prediction; // 올바른 키로 예측값을 저장
