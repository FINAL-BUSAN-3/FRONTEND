...
export default {
  ...
  methods: {
    async fetchModelSelect(processName) {
      try {
        const response = await axios.get(`http://ec2-18-215-52-54.compute-1.amazonaws.com:8000/model-deployment/model-select/${processName}`);
    ...
    async selectModel(model_info_id) {
      try {
        const response = await axios.get(`http://ec2-18-215-52-54.compute-1.amazonaws.com:8000/model-deployment/model-info/${model_info_id}`);
