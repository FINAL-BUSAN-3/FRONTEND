<template> 
  <div class="model-deployment">
    <div v-if="!uploadSuccess">
      ...
      <!-- 업로드 버튼 -->
      <button @click="uploadModel" class="upload-button">Upload</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    async uploadModel() {
      ...
      // FormData를 사용하여 파일과 기타 데이터를 전송
      const formData = new FormData();
      formData.append('selectedModel', this.selectedModel);
      formData.append('selectedProcess', this.selectedProcess);
      formData.append('file', this.file);

      try {
        await axios.post('/api/upload-model', formData, {
          headers: {
